...
export class MatriculaDetailComponent implements OnInit, OnDestroy {

  @Input() matriculaId!: number;
  @Output() close = new EventEmitter<void>();

  matricula: MatriculaResponse | null = null;
  loading = false;
  errorMsg = '';
  ...
  ngOnInit(): void {
    this.loading = true;
    this.matriculaService.findById(this.matriculaId)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (data) => { this.matricula = data; this.loading = false; },
        error: () => {
          this.errorMsg = 'Erro ao carregar matrícula.';
          this.loading = false;
        }
      });
  }
